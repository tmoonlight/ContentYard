...
namespace CY_System.Infrastructure
{
    ...
    public class BaseRepository<T> : IRepository<T> where T : class
    {
        ...
        public BaseRepository()
        {
            ...
            var attrArray = typeof(T).GetCustomAttributes(typeof(CY_System.DomainStandard.POCOAttribute), false);
            //            string newTableName = "";
            ...
            if (attrArray.Length > 0)
            {
                ...
                if (attr.DbConnName != null)
                {
                    this._connectionString = attr.DbConnName;
                }
                else
                {
                    this._connectionString = CY_SystemConsts.ConnectionString_conn;
                }
            }
        }
        ...
        /// <param name=""></param>
        /// <returns></returns>
        public virtual int Add(T model,out dynamic identity)
        {
            //取得实体上的连接
            using (var conn = GetConnection())
            {
                identity = conn.Insert(model);
                return 1;
            }
        }

        public virtual int AddList(IEnumerable<T> model)
        {
            //取得实体上的连接
            using (var conn = GetConnection())
            {
        ...
        /// <returns></returns>
        public virtual bool Exists(object predicate)
        {
            using (var conn = GetConnection())
            {
